# Clear Thinking FastMCP Server - Mental Model Server

"""
Mental Model cognitive tool server implementation.

This server applies structured mental model frameworks for problem analysis
including first principles, opportunity cost, error propagation, rubber duck,
Pareto principle, and Occam's razor approaches.

Agent: cognitive-tool-implementer
Status: ACTIVE - Mental Model server implementation complete
"""
...
class MentalModelServer(CognitiveToolBase[MentalModelInput, MentalModelOutput]):
    """Mental model cognitive tool server with FastMCP Context integration"""
    
    def __init__(self):
        """Initialize the mental model server"""
        super().__init__()
        self.tool_name = "Mental Models"
        self.version = "2.0.0"
    
    async def validate_input(self, data: MentalModelInput) -> bool:
        """Validate input data for mental model processing"""
        try:
            # Check if problem is suitable for the selected mental model
            is_compatible = MentalModelUtils.validate_model_compatibility(
                data.problem, 
                data.model_type
            )
            
            if not is_compatible:
                # Still allow, but may suggest alternative
                pass
            
            # Validate complexity requirements
            if data.complexity_level.value == "complex" and len(data.problem) < 100:
                return False
    ...
    async def process(
        ...
        """Process mental model analysis with Context integration"""
        
        await self.log_processing_start(data, ctx)
        
        try:
            # Route to specific mental model implementation
            if data.model_type == MentalModelType.FIRST_PRINCIPLES:
                result = await self._apply_first_principles(data, ctx)
            elif data.model_type == MentalModelType.OPPORTUNITY_COST:
                result = await self._apply_opportunity_cost(data, ctx)
            elif data.model_type == MentalModelType.ERROR_PROPAGATION:
                result = await self._apply_error_propagation(data, ctx)
            elif data.model_type == MentalModelType.RUBBER_DUCK:
                result = await self._apply_rubber_duck(data, ctx)
            elif data.model_type == MentalModelType.PARETO_PRINCIPLE:
                result = await self._apply_pareto_principle(data, ctx)
            elif data.model_type == MentalModelType.OCCAMS_RAZOR:
                result = await self._apply_occams_razor(data, ctx)
            else:
                raise ValueError(f"Unsupported mental model: {data.model_type}")
            ...
            ctx.error(f"Mental model processing failed: {e}")
            raise
    ...
    async def _apply_first_principles(
        ...
        ctx.progress(0.4, 1.0, "Breaking down to fundamental truths")
        
        # Simulate first principles analysis
        await asyncio.sleep(0.1)  # Simulate processing time
        
        # Generate fundamental elements
        fundamental_elements = await self._identify_fundamental_elements(data.problem, ctx)
        
        ctx.progress(0.6, 1.0, "Building up from fundamentals")
        ...
        ctx.progress(0.8, 1.0, "Generating recommendations")
        ...
        analysis = f"""First principles analysis reveals that {data.problem} can be broken down into fundamental elements: {', '.join(fundamental_elements)}. 

The core insight is that {fundamental_elements[0].lower()} represents the primary constraint, while {fundamental_elements[1].lower()} provides the foundation for building an effective solution.

By stripping away inherited assumptions and complexity, we can see that the problem fundamentally requires addressing {fundamental_elements[0].lower()} through {fundamental_elements[1].lower()}, rather than the more complex approaches typically considered."""
        
        confidence_score = 0.85
        
        ctx.progress(0.9, 1.0, "Finalizing first principles analysis")
    ...
    async def _apply_opportunity_cost(
        ...
        ctx.progress(0.4, 1.0, "Identifying alternatives and trade-offs")
        
        # Simulate opportunity cost analysis
        ...
        ctx.progress(0.6, 1.0, "Analyzing opportunity costs")
        ...
        analysis = f"""Opportunity cost analysis of {data.problem} reveals several key trade-offs that must be considered.

The primary trade-off involves balancing immediate needs against long-term sustainability. Quick solutions offer faster results but at the cost of future flexibility and maintenance burden. Comprehensive solutions provide better long-term outcomes but require significant upfront investment.

The analysis shows that the highest opportunity cost often comes from indecision itself - each day of delay incurs costs across all alternatives. The optimal approach likely involves a balanced strategy that delivers early value while maintaining long-term viability."""
    ...
    async def _apply_error_propagation(
        ...
        ctx.progress(0.4, 1.0, "Mapping error propagation paths")
        ...
        ctx.progress(0.6, 1.0, "Analyzing failure modes")
        ...
        analysis = f"""Error propagation analysis of {data.problem} reveals several critical failure pathways that could compound into major system issues.

The analysis identifies that errors typically originate at system boundaries (inputs, interfaces, configurations) and propagate through interconnected components. The key insight is that automated systems can propagate errors much faster than human operators can detect and respond to them.

Critical error paths include: {'; '.join(error_paths[:3])}. Each path represents a potential cascade that could transform minor issues into major system failures.

The most effective mitigation strategy involves implementing defense in depth: robust boundary validation, circuit breakers for isolation, comprehensive monitoring for early detection, and graceful degradation patterns."""
    ...
    async def _apply_rubber_duck(
        ...
        ctx.progress(0.4, 1.0, "Breaking down problem step-by-step")
        ...
        ctx.progress(0.6, 1.0, "Generating explanatory framework")
        ...
        analysis = f"""Rubber duck analysis of {data.problem} involves systematically explaining the problem as if teaching it to someone with no prior knowledge.

When we break down '{data.problem}' step by step:
...
This methodical explanation process often reveals that the real problem is different from the perceived problem, or that the solution was hidden by overly complex thinking."""
    ...
    async def _apply_pareto_principle(
        ...
        ctx.progress(0.4, 1.0, "Identifying critical 20% factors")
        
        await asyncio.sleep(0.1)
        
        # Generate critical factors based on problem analysis
        critical_factors = await self._identify_critical_factors(data.problem, ctx)
        
        ctx.progress(0.6, 1.0, "Analyzing impact distribution")
        ...
        analysis = f"""Pareto analysis of {data.problem} identifies the critical few factors that drive the majority of outcomes.

The analysis reveals that approximately 20% of factors account for 80% of the impact. In this case, the critical factors appear to be: {', '.join(critical_factors)}.

Focusing effort on {critical_factors[0]} alone could potentially address 80% of the problem, while the remaining factors, despite being numerous, contribute only marginally to the overall outcome.

This distribution suggests that traditional approaches spreading effort equally across all factors would be highly inefficient. Instead, concentrated focus on the critical few will yield dramatically better results."""
    ...
    async def _apply_occams_razor(
        ...
        ctx.progress(0.4, 1.0, "Identifying simplest explanations")
        
        await asyncio.sleep(0.1)
        
        # Generate simplified explanation
        simplified_explanation = await self._generate_simplified_explanation(data.problem, ctx)
        
        ctx.progress(0.6, 1.0, "Evaluating solution complexity")
        ...
        analysis = f"""Occam's Razor analysis of {data.problem} seeks the simplest explanation and solution that adequately addresses the core requirements.

The simplified explanation is: {simplified_explanation}

This analysis suggests that many proposed solutions may be unnecessarily complex. The simplest viable approach would involve addressing the core problem directly without additional layers of abstraction or sophistication.

While more complex solutions might handle edge cases better or provide more features, Occam's Razor suggests starting with the minimal viable solution and adding complexity only when proven necessary through actual use."""
    ...
    async def _identify_fundamental_elements(self, problem: str, ctx: Context) -> List[str]:
        """Identify fundamental elements for first principles analysis"""
        # In a real implementation, this would use NLP or domain expertise
        # For now, provide reasonable generic fundamental elements
    ...
    async def _identify_critical_factors(self, problem: str, ctx: Context) -> List[str]:
    ...
    async def _generate_simplified_explanation(self, problem: str, ctx: Context) -> str:
        """Generate simplified explanation for Occam's Razor analysis"""
        # Extract key concepts and provide simple explanation
        problem_lower = problem.lower()
