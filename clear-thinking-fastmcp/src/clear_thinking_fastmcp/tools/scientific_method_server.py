# Clear Thinking FastMCP Server - Scientific Method Server

"""
FastMCP server implementation for Scientific Method cognitive tool.

This server provides hypothesis-driven reasoning through:
...
from ..models.scientific_method import (
...
class ScientificMethodServer(CognitiveToolBase):
    """Server implementation for Scientific Method tool"""
    
    def __init__(self):
        super().__init__()
        self.tool_name = "Scientific Method"
        self.version = "1.0.0"
        self.description = "Hypothesis-driven systematic inquiry and evidence evaluation"
    ...
    async def process(
        ...
        """Process scientific method investigation"""
        
        start_time = time.time()
        
        try:
            await context.info(f"🔬 Starting Scientific Method investigation: {input_data.research_question}")
            await context.progress("Initializing scientific investigation", 0.0)
            
            # Phase 1: Generate hypotheses
            hypotheses = []
            if input_data.hypothesis_generation_enabled:
                await context.info("Phase 1: Generating testable hypotheses")
                hypotheses = await self._generate_hypotheses(
                    input_data, context
                )
                await context.progress("Generated hypotheses", 0.2)
            
            # Phase 2: Collect and evaluate evidence
            evidence_collected = []
            if input_data.evidence_evaluation_enabled:
                await context.info("Phase 2: Collecting and evaluating evidence")
                evidence_collected = await self._collect_evidence(
                    input_data, hypotheses, context
                )
                await context.progress("Collected evidence", 0.4)
            
            # Phase 3: Design experiments
            experiments = []
            if input_data.experiment_design_enabled:
                await context.info("Phase 3: Designing experiments for hypothesis testing")
                experiments = await self._design_experiments(
                    input_data, hypotheses, context
                )
                await context.progress("Designed experiments", 0.6)
            
            # Phase 4: Test hypotheses
            await context.info("Phase 4: Testing hypotheses against evidence")
            hypothesis_tests = await self._test_hypotheses(
                hypotheses, evidence_collected, input_data, context
            )
            await context.progress("Tested hypotheses", 0.8)
            
            # Phase 5: Construct theory if enabled
            theory = None
            if input_data.theory_construction_enabled:
                await context.info("Phase 5: Constructing theory from findings")
                theory = await self._construct_theory(
                    hypotheses, hypothesis_tests, evidence_collected, input_data, context
                )
                await context.progress("Constructed theory", 0.9)
            
            # Phase 6: Evaluate scientific rigor and generate conclusions
            await context.info("Phase 6: Evaluating scientific rigor and conclusions")
            (
                rigor_score,
                methodology_quality,
                evidence_strength,
            ...
            # Create output
            output = ScientificMethodOutput(
                problem=input_data.problem,
                complexity_level=input_data.complexity_level,
                confidence_score=self._calculate_overall_confidence(hypothesis_tests, evidence_collected),
                analysis=f"Scientific investigation completed with {len(hypotheses)} hypotheses, {len(evidence_collected)} evidence pieces, and {len(hypothesis_tests)} tests",
                ...
                scientific_confidence=self._calculate_scientific_confidence(hypothesis_tests, evidence_strength)
            )
            
            await context.progress("Scientific method investigation completed", 1.0)
            await context.info(f"✅ Scientific Method completed in {processing_time:.2f}s")
            
            return output
            
        except Exception as e:
            await context.error(f"Scientific Method failed: {str(e)}")
            raise
    ...
    async def _generate_hypotheses(
        ...
        for i, hyp_type in enumerate(hypothesis_types[:input_data.max_hypotheses]):
            hypothesis = await self._create_hypothesis(
                input_data.research_question,
                hyp_type,
                input_data.domain_knowledge,
                context
            )
            hypotheses.append(hypothesis)
            
            await context.debug(f"Generated {hyp_type.value} hypothesis: {hypothesis.statement}")
        
        # Add null hypothesis for statistical testing
        if len(hypotheses) > 0:
            null_hypothesis = await self._create_null_hypothesis(
                hypotheses[0], input_data, context
            )
            hypotheses.append(null_hypothesis)
        
        await context.debug(f"Generated {len(hypotheses)} total hypotheses")
        return hypotheses
    ...
    async def _create_null_hypothesis(
        ...
        return Hypothesis(
            statement=f"There is no significant relationship or effect in '{input_data.research_question}'",
    ...
    async def _collect_evidence(
        ...
        for evidence_type in evidence_types:
            for i, hypothesis in enumerate(hypotheses[:3]):  # Limit evidence per hypothesis
                evidence = await self._create_evidence(
                    hypothesis, evidence_type, input_data, context
                )
                evidence_pieces.append(evidence)
        
        # Add evidence from specified sources
        for source in input_data.evidence_sources:
        ...
        await context.debug(f"Collected {len(evidence_pieces)} pieces of evidence")
        ...
        for hypothesis in hypotheses[:3]:  # Limit experiments
            if hypothesis.hypothesis_type != HypothesisType.NULL_HYPOTHESIS:
                experiment = await self._create_experiment(
                    hypothesis, input_data, context
                )
                experiments.append(experiment)
        
        await context.debug(f"Designed {len(experiments)} experiments")
        return experiments
    ...
    async def _test_hypotheses(
        ...
        for hypothesis in hypotheses:
            ...
            test_result = await self._conduct_hypothesis_test(
                hypothesis, relevant_evidence, input_data, context
            )
            hypothesis_tests.append(test_result)
        
        await context.debug(f"Completed {len(hypothesis_tests)} hypothesis tests")
    ...
    async def _conduct_hypothesis_test(
        ...
        if len(evidence) >= 3 and evidence_quality > 0.6:
            statistical_significance = max(0.001, input_data.significance_threshold * (1 - confidence))
    ...
    async def _construct_theory(
        ...
        if len(supported_tests) < 2:
            await context.debug("Insufficient supported hypotheses for theory construction")
            return None
        ...
        return TheoryConstruction(
            theory_name=f"Integrated Theory for {input_data.research_question[:30]}...",
            ...
            scope=f"Applicable to problems similar to: {input_data.research_question}",
    ...
    async def _evaluate_scientific_process(
        ...
        rigor_factors = [
            len(hypotheses) / max(1, input_data.max_hypotheses),  # Hypothesis generation
    ...
    # Helper methods
    def _calculate_overall_confidence(self, hypothesis_tests: List[HypothesisTest], evidence: List[Evidence]) -> float:
        ...
        return sum(test_confidences) / len(test_confidences)
    
    def _calculate_scientific_confidence(self, hypothesis_tests: List[HypothesisTest], evidence_strength: float) -> float:
        """Calculate scientific confidence combining tests and evidence"""
        if not hypothesis_tests:
            return evidence_strength * 0.5
        
        test_confidence = self._calculate_overall_confidence(hypothesis_tests, [])
    ...
    def _identify_test_limitations(self, evidence: List[Evidence]) -> List[str]:
        ...
        return list(limitations) + common_limitations[:2]
    
    def _generate_test_recommendations(self, test_result: TestResult, confidence: float) -> List[str]:
