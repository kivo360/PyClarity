...
class ThoughtData(BaseModel):
    ...
    thought_type: str | None = Field(None, description="Type of reasoning step")
    confidence: float = Field(0.85, description="Confidence score")
    branch_id: str | None = Field(None, description="Branch identifier if branching")
    branch_from_thought: int | None = Field(None, description="Parent thought for branches")
    revises_thought: int | None = Field(None, description="ID of thought being revised")
...
class BaseSessionStore(ABC):
    ...
    @abstractmethod
    async def update_session(self, session_id: str, updates: dict[str, Any]) -> SessionData | None:
        """Update session data."""
        pass
...
class BaseThoughtStore(ABC):
    ...
    @abstractmethod
    async def update_thought(self, thought_id: int, updates: dict[str, Any]) -> ThoughtData | None:
